...
public class CoordinatorLeaderElection implements AutoCloseable {
    ...
    private void doInitLeaderServices(Runnable initLeaderServices) {
        try {
            ...
            Optional<ZkEpoch> optionalEpoch = zkClient.fenceBecomeCoordinatorLeader(serverId);
            if (optionalEpoch.isPresent()) {
                ZkEpoch epoch = optionalEpoch.get();
                coordinatorContext.setCoordinatorEpochAndZkVersion(
                        epoch.getCoordinatorEpoch(), epoch.getCoordinatorEpochZkVersion());
            } else {
                // Another coordinator has already updated the epoch — we lost the race.
                relinquishLeadership();
                throw new RuntimeException(
                        "Coordinator server "
                                + serverId
                                + " failed to fence epoch: another leader has taken over.");
            }
            ...
            // Re-throw RuntimeExceptions (including the one we just threw above) directly
            throw e;
        } catch (Exception e) {
